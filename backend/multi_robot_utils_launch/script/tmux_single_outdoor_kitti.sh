...
# tmux split-window -h -t $SESSION_NAME
tmux select-pane -t $SESSION_NAME:1.0
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; ros2 launch scan2shape_launch throttle_and_remap_kitti.launch.py" Enter
...
tmux select-pane -t $SESSION_NAME:1.3
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 2; ros2 launch scan2shape_launch process_cloud_node_outdoor_kitti_with_ns.launch.py" Enter
...
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 10; cd $BAG_DIR && ros2 bag play kitti-slide-slam-0-*.bag --clock -r $BAG_PLAY_RATE -s 0 /aft_mapped_to_init:=/Odometry /semantic_point_cloud:=/os_node/segmented_point_cloud_no_destagger_high_freq" Enter
