...
# ---------------------------------------------------------------------------
# Section O / P helpers: package.xml parsing + nearest-enclosing-pkg walker.
# These are used by the section O include-depends check and section P launch
# parameter-key cross-reference. They are intentionally simple string-level
# parsers (no XML library), tolerant of whitespace but not of CDATA or
# multi-line tag bodies, which this repo does not use.
# ---------------------------------------------------------------------------

# Print every `<depend>NAME</depend>` / `<build_depend>` / `<exec_depend>` /
# `<test_depend>` / `<buildtool_depend>` NAME from a package.xml, one per line,
# stripped of whitespace, sorted + uniq'd. Returns nothing if the file is
